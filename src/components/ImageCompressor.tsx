    ...
    try {
      ...
      if (downloadMode === "zip") {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          try {
            const { blob, originalSize, compressedSize, originalDimensions } =
            ...
            results.push({
              originalName: file.name,
              error:
                error instanceof Error ? error.message : "Compression error",
            });
            failedConversions++;
          }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-red-900 to-pink-900">
      <div className="max-w-6xl mx-auto p-6 space-y-8">
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-r from-red-600/30 to-pink-600/30 rounded-3xl blur-xl transition-opacity duration-300 ${
          ...
          <div className="bg-gray-800/80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border border-gray-700/20">
            ...
            <div className="max-h-80 overflow-y-auto space-y-3 custom-scrollbar">
              {selectedFiles.map((file, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-700/50 to-red-900/30 rounded-2xl border border-gray-600/50 hover:shadow-lg hover:border-gray-500/50 transition-all duration-200"
                >
