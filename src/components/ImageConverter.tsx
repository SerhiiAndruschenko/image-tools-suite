      ...
      if (invalidFiles.length > 0) {
        setError(
          `Unsupported file formats: ${invalidFiles
            .map((f) => f.name)
            .join(", ")}. Only JPEG, JPG and PNG are supported`
        );
        return;
      }
    ...
    try {
      ...
      if (downloadMode === "zip") {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          try {
            const { blob, originalSize, convertedSize } = await convertToWebP(
              file
            );
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-gray-300 max-w-2xl mx-auto">
            Convert images to optimized WebP format locally in your browser - no
            server limits!
          </p>
        </div>
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-r from-blue-600/30 to-purple-600/30 rounded-3xl blur-xl transition-opacity duration-300 ${
